...
export const SplitFlapText: React.FC<SplitFlapTextProps> = ({
  ...
  const componentStyle: React.CSSProperties = {
    ...
    ['--split-flap-radius' as string]: toCssUnit(tileRadius),
    ['--split-flap-gap' as string]: toCssUnit(gap),
    ['--split-flap-font-size' as string]: toCssUnit(fontSize),
    ['--split-flap-flip-duration' as string]: `${Math.max(0.04, Number(flipDuration) || 0.12)}s`,
    ...style
  };
    ...
    >
      {tiles.map((tile, index) => (
        <span className="split-flap-text__tile" aria-hidden="true" key={`${index}-${tiles.length}`}>
          ...
          {tile.flipping && (
            <React.Fragment key={`flip-${index}-${tile.tick}`}>
